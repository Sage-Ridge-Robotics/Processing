...
Exercise 18. The sketch receives luminosity data in lux over the 
serial link from an Arduino with an attached luminosity sensor.
The associated Arduino sketch can be configured to report visible
...
// Import the serial library. Unlike Arduino, serial 
...
// Read the reported lux value and draw an ellipse the size 
// and hue of which are scaled to the lux value. 
// Lux data is used for hue.
void draw() {
